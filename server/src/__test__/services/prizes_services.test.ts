...
describe("PrizesService", () => {
  ...
  test("createPrize: providerId no existe", async () => {
    ...
    const result = await prizesService.createPrize({ name: "TV", providerId: 99 });
    expect(result).toBeNull();
  });
  ...
  test("createPrize: raffleId no existe", async () => {
    ...
    const result = await prizesService.createPrize({ name: "TV", providerId: 3, raffleId: 88 });
    expect(result).toBeNull();
  });
  ...
  test("createPrize: crea correctamente", async () => {
    ...
    const result = await prizesService.createPrize({ name: "TV", providerId: 1, raffleId: 2 });
    expect(result).toEqual({ message: "Premio creado correctamente", data: created });
  });

  test("updatePrize: premio no encontrado", async () => {
    prizeRepo.findOne.mockResolvedValue(null);
    await expect(prizesService.updatePrize(1, {})).rejects.toThrow("Premio no encontrado");
  });

  test("updatePrize: provider no existe", async () => {
    prizeRepo.findOne.mockResolvedValue({ id: 1 });
    providerRepo.findOneBy.mockResolvedValue(null);
    await expect(prizesService.updatePrize(1, { providerId: 99 })).rejects.toThrow("Proveedor no encontrado");
  });
  ...
  test("updatePrize: actualiza correctamente", async () => {
    ...
    const result = await prizesService.updatePrize(1, { name: "New", providerId: 5 });
    expect(result).toEqual({ message: "Premio actualizado correctamente", data: { id: 1, name: "New" } });
  });

  test("deletePrize: no existe", async () => {
    prizeRepo.findOne.mockResolvedValue(null);
    await expect(prizesService.deletePrize(9)).rejects.toThrow("Premio no encontrado");
  });

  test("deletePrize: rifa activa", async () => {
    prizeRepo.findOne.mockResolvedValue({ raffle: { status: "active", tickets: [] } });
    await expect(prizesService.deletePrize(1)).rejects.toThrow("No se puede eliminar el premio porque la rifa está activa");
  });
  ...
  test("getWinners: retorna datos formateados", async () => {
    ...
    expect(result[0].winner_user).toEqual({
