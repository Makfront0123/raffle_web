...
export function useRaffles() {
  ...
  // --- Crear rifa con objeto temporal
  const createRaffle = useCallback(
    async (
      ...
      try {
        ...
        // --- Agregar temporal y mostrar toast
  ...
  // --- Funciones de modificación de rifas
...
export function useRaffles() {
  ...
  // --- Funciones que modifican rifas y refrescan
  const handleDeleteRaffle = useCallback(
    async (id: number) => {
      if (!token) return;
      await deleteRaffle(id, token);
  ...
  // --- Rifas filtradas (solo activas y ordenadas por fecha)
  const filteredRaffles = useMemo(() => {
    return storeRaffles
      .filter(r => r.status?.toLowerCase().trim() === "active")
      .sort((a, b) => {
